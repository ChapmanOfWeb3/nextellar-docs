...
<Note type="info">
  This hook ships in the `default` and `js` templates. It is not currently
  included in the `minimal` or `defi` templates.
</Note>
...
export default function TransactionHistoryList({
  ...
  const { items, loading, error, fetchNextPage, hasMore, refresh } =
...
Render the current page and let the user request the next cursor only when Horizon reports that more records are available:

```tsx
import { useTransactionHistory } from '@/hooks/useTransactionHistory';

export function PaginatedHistory({ publicKey }: { publicKey?: string }) {
  const { items, loading, error, fetchNextPage, hasMore, refresh } =
    useTransactionHistory(publicKey, { pageSize: 20, type: 'payments' });
  ...
  if (error) {
    return (
      <div>
        <p role="alert">Unable to load transactions: {error.message}</p>
  ...
  return (
    <section>
      ...
      {!loading && items.length === 0 && <p>No transactions found.</p>}
    </section>
  );
}
```
...
- [`useOfferBook`](/docs/hooks/use-offer-book) - Query DEX activity
